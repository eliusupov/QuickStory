...
| 23-70 | **Five quests still gate on quest-records nothing writes.** `Quest.canStart` AND `canComplete` both call `canQuestByInfoProgress`; `autoStart`/`autoComplete` do NOT bypass it (`QuestActionHandler` cases 1/2/4/5 guard *before* `start()` ever sees the flag). Remaining: **22556**(22598="1") location settled - `Say.img/22556/0/yes/2` "go no further than #m106010102#" - but that map has **no `onUserEnter`/`onFirstUserEnter` node at all**, and the reportable finding is the puppet door (`106010102/portal/8`, script `evanDollGR`), so map-arrival vs portal-touch is **UNKNOWN**; also unreachable until `evanGolemDoor.js` exists. **22557**(22598="2") **UNKNOWN and the data actively refuses** - Camila 1012108 is placed in exactly one map in all of Map.wz (100000000, Henesys town), no temple map has a hook, and `Check.img/22557/1` declares no `mob` and no `item` node. **22580**(22599="1"/"2") both locations settled (914100000 landing; 914100020, the room with the ten `stopIceWall` portals) but **neither map declares a non-empty hook** - map-entry vs portal is UNKNOWN. **22589**(22600="1") UNKNOWN between `outSDI` and 22588's `autoComplete`; (22604="1") best-settled of the five - re-enter 914100021, the only cave with Afrien placed, whose `onUserEnter` is `evanTogether` and that file **already exists** (but is shared with 100030102, so it must be gated on map id - that gating is inference, not data). **22591**(22601="1") UNKNOWN - Afrien 1205000 IS placed on 914100021, but `Say.img/22591/0` is empty because the accept text lived in server script `q22591s`; candidates are `q22591s` and `outAfrienMemory`/900030000 "Afrien's Memory". Mir 1013000 is placed on no map by design and needs none - `QuestActionHandler.MIR` already accepts a summoned dragon for proximity. | QUEUED / UNKNOWN |

...
| **Evan skills** | 8 of 12 closed. Mounts are a 3-line fix (R14); Magic Resistance is blocked on a client disassembly (R22); two are WONTFIX with reasons. |
| **Evan character stats** | **Two live defects the old tracker never recorded**: an Evan gains 6 MP per AP point instead of 18 (R41), and there is no AP-reset floor (R42). Plus persisted stat damage on the owner's character (R43). |
| **Drops, cards, maker** | Closed. 214 mob-drop rows, 39/39 monster cards, 6/6 maker. Residue is two `UPDATE`s and seven stale card rows, all owner-gated (R19, R20), plus a contradiction between two of our own changeSets about mob 8300007 (R49). |

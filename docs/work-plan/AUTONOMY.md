...
Default budget per task: dispatch (~300) + implement report (~400) + ledger write (~200) ≈ **900
tokens**. Add a review report only when the owner requests independent review.
...
3. Dispatch the agent named in the row's `agent` column, with the **`/implement`** skill, pointed at
   the owner's task brief or `docs/work-plan/tickets/NN-*.md`, plus the report contract.
   **That agent commits its own work.** No new ticket is required.
4. Write its verdict to the ledger.
5. Only if the owner requested review, dispatch a `gp-opus-high` agent with the **`/code-review`**
...
One task at a time: dispatch the agent named in its `agent` column with /implement, let it
commit its own work, write the verdict to the ledger. Create specs or tickets and run /code-review
only when I ask for each step. Do not open project files yourself, do not

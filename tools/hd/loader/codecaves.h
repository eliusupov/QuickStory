...
// v84 BODY EDIT 1 of 2 -- REDESIGNED, not re-pointed.
// v83 displaced `push 0x16 ; movsd ; push 0` (5 bytes). v84 recompiled this call from a
// vtable call taking two ZXStrings BY VALUE into a direct thiscall taking them BY
// POINTER, so that sequence does not exist in v84 at all. The v84 site displaces
// `push 0x16 ; push edi ; lea ecx,[esi+0xD04]` = 2+1+6 = 9 bytes -- the same shape as
// AdjustStatusBarInput below. 9 is the only whole-instruction tiling >= 5 there; phase 1
// proposed 3 NOPs, which cannot hold a 5-byte E9.
__declspec(naked) void AdjustStatusBarBG() {
	__asm {
		push nStatusBarY
		push edi
		lea ecx, [esi + HD_AdjustStatusBarBG_MEMBER]
		jmp dword ptr[dwStatusBarBackgroundPosRetn]
	}
}

// v84 BODY EDIT 2 of 2 -- the CUIStatusBar member moved 0x0CD0 -> 0x0D08.
// The shift is NOT uniform across the struct: members below ~0xC40 moved +0x30, those
// above moved +0x38. Both figures come from verify.py's displaced-sequence diff, not
// from an assumption.
__declspec(naked) void AdjustStatusBarInput() {
	__asm {
		push nStatusBarY
		push edi
		lea ecx, [esi + HD_AdjustStatusBarInput_MEMBER]
